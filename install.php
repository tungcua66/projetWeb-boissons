...
$select_database_exists = $database_boissons->query("select name from sqlite_master;");
$database_exists = $select_database_exists->fetchArray();
if (!$database_exists) {
    /*   load resource file   */
    include "Donnees.inc.php";
    /*   create hierarchy database   */
    ...
    /*   create recipes and indexes databases   */
    ...
    $database_boissons->exec("create table if not exists login (user_id integer primary key autoincrement, username text, password_hash text);");
    /*   create basket database   */
    $database_boissons->exec("create table if not exists basket (user_id integer, rcp_id integer, unique(user_id, rcp_id));");
}
